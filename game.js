...
function addAgitatorWithCircle(scene, xPosition) {
  // Создаем фиолетовый agitator
  const agitator = scene.add.circle(0, 0, 10, 0x800080); // Радиус 10

  // Создаем зеленый круг
  const greenCircle = scene.add.circle(0, 0, 40, 0x00ff00, 0.2); // Радиус 40, прозрачность 20%

  // Создаем контейнер, центр которого будет определять позицию обоих объектов
  const agitatorContainer = scene.add.container(xPosition, 0, [
    greenCircle,
    agitator,
  ]);

  // Добавляем физическое тело к контейнеру
  scene.physics.add.existing(agitatorContainer);
  agitatorContainer.body.setVelocityY(100); // Задаем скорость для всего контейнера

  // Добавляем в массив
  agitators.push(agitatorContainer);
  agitatorActive = true;
  currentAgitator = agitatorContainer;

  return agitatorContainer;
}
...
function spawnAgitator(scene) {
  ...
  // Создаем agitator с зеленым кругом через addAgitatorWithCircle
  const agitatorContainer = addAgitatorWithCircle(scene, xPosition);

  // Добавляем в массив agitators и делаем его активным
  agitators.push(agitatorContainer);
  agitatorActive = true;
  currentAgitator = agitatorContainer;

  // Устанавливаем удаление agitator через 8 секунд
  scene.time.delayedCall(8000, () => {
    agitatorContainer.destroy(); // Уничтожаем контейнер
...
function update3(time, delta, scene) {
  ...
  agitators.forEach((agitatorContainer) => {
    const agitator = agitatorContainer.agitator;
    const greenCircle = agitatorContainer.greenCircle;

    // Синхронизируем координаты (если нужно)
    // greenCircle.x = agitator.x;
    // greenCircle.y = agitator.y;
  });
}
