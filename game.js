...
function update(time, delta) {
  ...
  //   // Игнорируем зрителей, которые уже заняли своё место
  //   if (
  //     !isSpectatorAtSeat(spectator) &&
  //     checkCollision(circleBody, spectator)
  //   ) {
...
//   // Ищем соответствующее место для зрителя
//   const seat = seats.find(
//     (seat) => seat.x === spectator.x && seat.y === spectator.y && seat.occupied
//   );

//   return !!seat; // Возвращаем true, если место занято
// }
// function isSpectatorAtSeat(spectator) {
//   // Ищем место для зрителя с допуском по расстоянию
//   const seat = seats.find(
//     (seat) =>
//       Phaser.Math.Distance.Between(spectator.x, spectator.y, seat.x, seat.y) <=
//         seatRadius && seat.occupied
//   );
