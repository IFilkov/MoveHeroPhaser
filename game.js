...
//   }
// }
// Функция для перемещения к случайным зрителям
// function moveRandomly(time, delta) {
//   // Если прошло достаточно времени, выбираем нового случайного зрителя
//   if (!targetSpectator || time - lastDirectionChange > changeDirectionTime) {
//     targetSpectator = Phaser.Utils.Array.GetRandom(spectators); // Выбираем случайного зрителя
//     lastDirectionChange = time; // Обновляем время последней смены цели
//   }

//   // Если есть целевой зритель, двигаем к нему
//   if (targetSpectator) {
//     const dx = targetSpectator.x - circleBody.x;
//     const dy = targetSpectator.y - circleBody.y;
//     const distance = Math.sqrt(dx * dx + dy * dy);

//     // Если еще не достигли цели, продолжаем двигаться
//     if (distance > 1) {
//       const angle = Math.atan2(dy, dx);
//       circleBody.x += Math.cos(angle) * speed * (delta / 1000);
//       circleBody.y += Math.sin(angle) * speed * (delta / 1000);
...
//       targetSpectator = Phaser.Utils.Array.GetRandom(spectators);
//     }
//   }
// }
...
//   return seats.some(
//     (seat) => seat.x === spectator.x && seat.y === spectator.y && seat.occupied
//   );
// }
function isSpectatorAtSeat(spectator) {
