...
// Scene3
let countdownText;
let countdownTime = 2 * 60; // 6 минут в секундах
...
let bluePioples = []; // Для pioples, перекрашенных в синий и занявших место слева
let redPioples = []; // Для pioples, перекрашенных в красный и занявших место справа
const occupiedBluePioples = new Set(); // Синие, которые уже нашли пару
const occupiedRedPioples = new Set(); // Красные, которые уже нашли пару

// const availableBluePioples = []; // Синие, которые ищут пару
// const availableRedPioples = []; // Красные, которые ищут пару
let availableBluePioples = []; // Изменяем с const на let
let availableRedPioples = []; // Изменяем с const на let
...
function spawnPiople(scene) {
  ...
  piople.isTargeting = false; // Сброс флага
  piople.active = true; // Отмечаем как активный
  ...
  scene.time.delayedCall(1000, () => {
    ...
    if (shouldTakeSpot) {
      ...
      if (freeSpot) {
        ...
        bluePioples.push(piople); // Добавляем в массив
        console.log("Добавлен в синих:", piople);
        ...
        const checkArrival = scene.time.addEvent({
          ...
          callback: () => {
            ...
            if (
              ...
              piople.isTargeting = false; // Сброс флага
              occupiedBluePioples.delete(piople);
              occupiedRedPioples.delete(piople);
            }
          },
          loop: true,
        });
...
function spawnAgitator(scene) {
  ...
  // Заполняем targetsQueue активными pioples
  startAgitatorTargetingPioples(agitator, pioples);
...
function moveToTarget(entity, speed) {
  if (!currentAgitator) return; // Если agitator отсутствует, выходим

  // Логика движения к agitator
  if (!agitatorCollidedWithCircleBody && !agitatorCollidedWithEnemy3) {
    const dx = currentAgitator.x - entity.x;
    const dy = currentAgitator.y - entity.y;
    const distance = Math.sqrt(dx * dx + dy * dy);
    entity.body.setVelocity((dx / distance) * speed, (dy / distance) * speed);

    // Проверка на коллизию
    if (Phaser.Geom.Intersects.CircleToCircle(entity, currentAgitator)) {
      if (entity === circleBody) {
        agitatorCollidedWithCircleBody = true; // Устанавливаем флаг
      } else if (entity === enemy3) {
        agitatorCollidedWithEnemy3 = true; // Устанавливаем флаг
      }

      // Сбрасываем цель и переключаемся на pioples
      entity.body.setVelocity(0); // Останавливаем объект
      currentAgitator = null; // Сбрасываем текущий agitator

      entity.scene.time.delayedCall(100, () => {
        if (entity === circleBody) {
          agitatorCollidedWithCircleBody = false;
          moveToClosestPiople(circleBody, pioples, speed); // Снова ищем pioples
        } else if (entity === enemy3) {
          agitatorCollidedWithEnemy3 = false;
          moveEnemy3ToClosestPiople(speed); // Снова ищем pioples
        }
      });
    }
  }
...
// function movePiopleToTarget(piople, target, isBlue) {
//   if (!piople || !piople.body || !target || !target.body) return;

//   piople.isTargeting = true; // Помечаем как активного
//   target.isTargeted = true; // Помечаем цель как выбранную

//   const dx = target.x - piople.x;
//   const dy = target.y - piople.y;
//   const distance = Math.sqrt(dx * dx + dy * dy);

//   const speed = 100;
//   piople.body.setVelocity((dx / distance) * speed, (dy / distance) * speed);

//   const checkCollision = piople.scene.time.addEvent({
//     delay: 50,
//     callback: () => {
//       if (!piople.active || !target.active) {
//         checkCollision.remove();
//         return;
//       }

//       // Проверка коллизии
//       if (checkPiopleCollision(piople, target)) {
//         piople.body.setVelocity(0);
//         target.body.setVelocity(0);

//         // Обновляем флаги
//         piople.isTargeting = false;
//         target.isTargeted = false;

//         // Добавляем в занятые
//         if (isBlue) {
//           occupiedBluePioples.add(piople);
//           occupiedRedPioples.add(target);
//         } else {
//           occupiedRedPioples.add(piople);
//           occupiedBluePioples.add(target);
//         }

//         checkCollision.remove();
//         updateAvailablePioples(); // Обновляем доступные
//       }
//     },
//     loop: true,
//   });
...
function movePiopleToTarget(piople, target, isBlue) {
  ...
  const checkCollision = piople.scene.time.addEvent({
    ...
    callback: () => {
      if (checkPiopleCollision(piople, target)) {
        handleCollisionAfterReach(piople, target, isBlue);
        checkCollision.remove();
      }
    },
    loop: true,
...
// function updateAvailablePioples() {
//   availableBluePioples.length = 0;
//   availableRedPioples.length = 0;

//   bluePioples.forEach((piople) => {
//     if (
//       !occupiedBluePioples.has(piople) &&
//       !piople.isTargeting &&
//       piople.active
//     ) {
//       availableBluePioples.push(piople);
//     }
//   });

//   redPioples.forEach((piople) => {
//     if (
//       !occupiedRedPioples.has(piople) &&
//       !piople.isTargeting &&
//       piople.active
//     ) {
//       availableRedPioples.push(piople);
//     }
//   });
// }
function updateAvailablePioples() {
  availableBluePioples = bluePioples.filter(
    (piople) =>
      !occupiedBluePioples.has(piople) && !piople.isTargeting && piople.active
  );

  availableRedPioples = redPioples.filter(
    (piople) =>
      !occupiedRedPioples.has(piople) && !piople.isTargeting && piople.active
  );
}

function debugPioplesState() {
  console.log("Available Blue Pioples:", availableBluePioples);
  console.log("Available Red Pioples:", availableRedPioples);

  bluePioples.forEach((piople, index) => {
    console.log(`Blue Piople ${index}:`, {
      active: piople.active,
      isTargeting: piople.isTargeting,
      inOccupied: occupiedBluePioples.has(piople),
    });
  });

  redPioples.forEach((piople, index) => {
    console.log(`Red Piople ${index}:`, {
      active: piople.active,
      isTargeting: piople.isTargeting,
      inOccupied: occupiedRedPioples.has(piople),
    });
  });
}

// function findClosestOppositePiople(piople, targets) {
//   let closestPiople = null;
//   let minDistance = Infinity;

//   targets.forEach((target) => {
//     if (
//       !target.active || // Игнорируем неактивных
//       occupiedBluePioples.has(target) || // Игнорируем занятых
//       occupiedRedPioples.has(target) || // Игнорируем занятых
//       target.isTargeted // Игнорируем уже выбранные цели
//     ) {
//       return;
//     }
...
function findClosestOppositePiople(piople, targets) {
  ...
  targets.forEach((target) => {
    if (
      target.active &&
      !target.isTargeted &&
      !occupiedBluePioples.has(target) &&
      !occupiedRedPioples.has(target)
    ) {
      const distance = Phaser.Math.Distance.Between(
        piople.x,
        piople.y,
...
function updatePiopleInteractions() {
  availableBluePioples.forEach((bluePiople) => {
    const target = findClosestOppositePiople(bluePiople, availableRedPioples);
    if (target) movePiopleToTarget(bluePiople, target, true);
  });

  availableRedPioples.forEach((redPiople) => {
    const target = findClosestOppositePiople(redPiople, availableBluePioples);
    if (target) movePiopleToTarget(redPiople, target, false);
  });
}

function checkPiopleCollision(piople, target) {
  if (!piople || !target) return false;
  const distance = Phaser.Math.Distance.Between(
    piople.x,
    piople.y,
    target.x,
  ...
  const combinedRadius = piople.displayWidth / 2 + target.displayWidth / 2 + 2; // Добавим небольшой отступ
  return distance <= combinedRadius;
}

// function startPioplesInteraction() {
//   updateAvailablePioples(); // Обновляем списки доступных

//   availableBluePioples.forEach((bluePiople) => {
//     const target = findClosestOppositePiople(bluePiople, availableRedPioples);
//     if (target) {
//       movePiopleToTarget(bluePiople, target, true); // Синий ищет красного
//     }
//   });
...
//     const target = findClosestOppositePiople(redPiople, availableBluePioples);
//     if (target) {
//       movePiopleToTarget(redPiople, target, false); // Красный ищет синего
//     }
//   });
// }
function startPioplesInteraction() {
  updateAvailablePioples();

  availableBluePioples.forEach((bluePiople) => {
    const target = findClosestOppositePiople(bluePiople, availableRedPioples);
    if (target) {
      movePiopleToTarget(bluePiople, target, true); // Синий движется к красному
    }
  });
  ...
  availableRedPioples.forEach((redPiople) => {
    const target = findClosestOppositePiople(redPiople, availableBluePioples);
    if (target) {
      movePiopleToTarget(redPiople, target, false); // Красный движется к синему
    }
  });

  // Убедитесь, что процесс продолжается, пока есть доступные pioples
  if (availableBluePioples.length > 0 || availableRedPioples.length > 0) {
    setTimeout(startPioplesInteraction, 500); // Рекурсивный вызов
  }
}

function moveEntitiesToBottom() {
  const bottomPositionY = config.height - 100; // Позиция над таймером
  const centerX = config.width / 2; // Центр экрана по X

  // Перемещаем circleBody
  circleBody.setPosition(centerX - 50, bottomPositionY); // Ставим внизу слева
  circleBody.body.setVelocity(0); // Останавливаем движение

  // Перемещаем enemy3
  enemy3.setPosition(centerX + 50, bottomPositionY); // Ставим внизу справа
  enemy3.body.setVelocity(0); // Останавливаем движение
}
function disableEntityMovement(entity) {
  entity.body.setVelocity(0); // Останавливаем движение
  entity.body.moves = false; // Полностью отключаем физическое перемещение
}
function updateCountdown() {
  if (countdownTime > 0) {
    countdownTime--; // Уменьшаем время на 1 секунду
    countdownText.setText(formatTime(countdownTime)); // Обновляем текст
  } else {
    // Когда таймер заканчивается
    countdownText.setText("Time's up!");
    this.time.removeAllEvents(); // Останавливаем все таймеры
    timerEnded = true; // Устанавливаем флаг

    // Таймер для обновления движения pioples
    this.time.addEvent({
      delay: 1000, // Интервал обновления
      callback: () => {
        if (timerEnded) {
          resetPioplesAfterTimer();
          moveEntitiesToBottom();
          // Вызов функции для обоих объектов
          disableEntityMovement(circleBody);
          disableEntityMovement(enemy3);
          startPioplesInteraction();
          debugPioplesState();
        }
      },
      loop: true,
    });
    // Запускаем движение pioples
    // startPioplesInteraction();
  }
}

function formatTime(seconds) {
  const minutes = Math.floor(seconds / 60); // Количество минут
  const secs = seconds % 60; // Оставшиеся секунды
  return `${minutes.toString().padStart(2, "0")}:${secs
    .toString()
    .padStart(2, "0")}`; // Форматируем в "MM:SS"
}

const leftBoundary = roadMargin; // Левая граница дороги
const rightBoundary = config.width - roadMargin; // Правая граница дороги
...
function create3() {
  ...
  countdownText = this.add.text(
    this.scale.width / 2, // Центр экрана по ширине
    this.scale.height - 50, // Немного выше нижней границы экрана
    formatTime(countdownTime), // Форматируем время в "MM:SS"
    {
      font: "32px Arial", // Шрифт и размер текста
      fill: "#B22222", // Цвет текста
    }
  );
  countdownText.setOrigin(0.5); // Выравниваем текст по центру

  // Запуск обновления текста таймера каждую секунду
  this.time.addEvent({
  ...
  this.physics.add.overlap(circleBody, agitators, (circleBody, agitator) => {
    ...
    // Переключаем circleBody обратно на pioples
    circleBody.scene.time.delayedCall(100, () => {
      agitatorCollidedWithCircleBody = false; // Сброс флага
      moveToClosestPiople(circleBody, pioples, circleBodySpeed);
    });
  });
  ...
  this.physics.add.overlap(enemy3, agitators, (enemy3, agitator) => {
    ...
    // Переключаем enemy3 обратно на pioples
    enemy3.scene.time.delayedCall(100, () => {
      agitatorCollidedWithEnemy3 = false; // Сброс флага
      moveEnemy3ToClosestPiople(enemy3Speed);
    });
  });
  this.input.on("pointermove", (pointer) => {
    mousePos.x = pointer.x;
  ...
  this.physics.world.setBounds(
    leftBoundary, // x координата левой границы
    0, // Верхняя граница
    rightBoundary - leftBoundary, // Ширина дороги
    config.height // Высота экрана
  );

  // Привязываем границы к объектам
  circleBody.body.setCollideWorldBounds(true);
  enemy3.body.setCollideWorldBounds(true);

  // Настройка отскока
  circleBody.body.setBounce(1); // Полный отскок
  enemy3.body.setBounce(1); // Полный отскок
}
  ...
  enemy.scene.time.delayedCall(1000, () => {
    ...
    if (shouldTakeSpot) {
      ...
      if (freeSpot) {
        ...
        redPioples.push(piople); // Добавляем в массив
        console.log("Добавлен в красных:", piople);
...
let targetsQueue = []; // Очередь целей для agitator
let currentTarget = null; // Текущая цель agitator
...
function startAgitatorTargetingPioples(
  ...
  targetsQueue = pioples.filter(
    (piople) => piople.active && !ignorePioples.includes(piople)
  );

  // Сортируем по расстоянию от agitator
  targetsQueue.sort((a, b) => {
    ...
    return distanceA - distanceB; // Ближайшие pioples идут первыми
  });
  ...
  if (!currentTarget) {
    console.log("No targets available for agitator.");
  }
...
function updateAgitatorMovement(agitator, speed) {
  if (!agitator || !agitator.active) return; // Добавлена проверка на null

  if (agitatorMode === "down") {
    agitator.body.setVelocityY(100); // Движение вниз
...
function handleAgitatorReachedTarget(agitator, target) {
  if (!target || !target.active) return; // Проверяем, что цель существует и активна

  if (agitator.collidedWithCircleBodyFirst) {
    // Если circleBody столкнулся первым, piople занимает место слева
    placePiopleOnLeft(target);
  } else {
    // Если enemy3 столкнулся первым, piople занимает место справа
    placePiopleOnRight(target);
  }
  target.active = false; // Помечаем цель как обработанную
  target.hasCollided = true;
  target.isTargeting = false; // Сбрасываем флаг для дальнейшей логики
}
...
function placePiopleOnRight(piople) {
  ...
  if (freeSpot) {
    ...
    redPioples.push(piople); // Добавляем в массив
    console.log("Добавлен в красных:", piople);
    console.log("Красные pioples:", redPioples);
  }
}

function resetPioplesAfterTimer() {
  bluePioples.forEach((piople) => {
    piople.hasCollided = false;
    piople.active = true; // Снова делаем доступными
    piople.isTargeting = false;
  });
  ...
  updateAvailablePioples(); // Обновляем списки доступных целей
}
...
function handleCollisionAfterReach(piople, target, isBlue) {
  if (!piople || !piople.body) {
    // console.warn("Piople не имеет физического тела или undefined:", piople);
  ...
  if (isBlue) {
    occupiedBluePioples.add(piople);
    occupiedRedPioples.add(target);
    availableBluePioples = availableBluePioples.filter((p) => p !== piople);
    availableRedPioples = availableRedPioples.filter((p) => p !== target);
  } else {
    occupiedRedPioples.add(piople);
    occupiedBluePioples.add(target);
    availableRedPioples = availableRedPioples.filter((p) => p !== piople);
    availableBluePioples = availableBluePioples.filter((p) => p !== target);
  }
}
...
function update3(time, delta, scene) {
  ...
  if (agitatorActive) {
    updateAgitatorMovement(currentAgitator, 400); // Скорость agitator
  }
  updatePiopleInteractions();
}
